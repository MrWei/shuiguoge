    ...
    public function index() {
        ...
        if ($act == 'js') {
            ...
            $sql = 'SELECT ad.ad_id, ad.ad_name, ad.ad_link, ad.ad_code ' .
                    'FROM ' . $this->model->pre . 'touch_ad AS ad ' .
                    'LEFT JOIN ' . $this->model->pre . 'touch_ad_position AS p ON ad.position_id = p.position_id ' .
            ...
            if (!empty($ad_info)) {
                ...
                switch ($_GET['type']) {
                    case '0':
                        ...
                        $str = '<a href="' . $url . url('affiche/index', array('ad_id' => $ad_info['ad_id'])) . '&from=' . $_GET['from'] . '&uri=' . urlencode($ad_info['ad_link']) . '" target="_blank">' .
