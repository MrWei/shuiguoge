...
?>
 ...
 <?php
    $GLOBALS['smarty']->assign('ad',get_adv('首页-分类ID'.$GLOBALS['smarty']->_var['category'].'-广告1'));
    ?>
	<?php if ($this->_var['ad']): ?>
    ...
    <?php
    $GLOBALS['smarty']->assign('ad',get_adv('首页-分类ID'.$GLOBALS['smarty']->_var['category'].'-广告2'));
    ?>
	<?php if ($this->_var['ad']): ?>
...
?>
		...
		<?php $_from = $this->_var['filter_attr_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'filter_attr_0_49617600_1428040566');if (count($_from)):
    foreach ($_from AS $this->_var['filter_attr_0_49617600_1428040566']):
?>
        <dl class="xm-filter-list category-filter-list clearfix">
          <dt><?php echo htmlspecialchars($this->_var['filter_attr_0_49617600_1428040566']['filter_attr_name']); ?>：</dt>
          <dd>
            <ul>
			<?php $_from = $this->_var['filter_attr_0_49617600_1428040566']['attr_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'attr');if (count($_from)):
