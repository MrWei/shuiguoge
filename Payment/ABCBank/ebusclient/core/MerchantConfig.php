	...
	private static function bundle() {
		if (!self :: $iIsInitialed) {
			...
			if (!$tIniFile) {
				...
				$tIniFile = dirname(__FILE__) . '/../TrustMerchant.php';
			}
			//self :: $iResourceBundle = @ parse_ini_file($tIniFile);
			self :: $iResourceBundle = require_once ($tIniFile);
// 			$tIniFile1 = require_once (dirname(__FILE__) . '/../TrustMerchant.php');
// 			echo "<pre>";
// 			print_r(self::$iResourceBundle);
// 			print_r($tIniFile1);
// 			exit();
